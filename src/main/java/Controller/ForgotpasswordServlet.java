	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (list != null) {
				...
				response.sendRedirect(request.getContextPath() + "/ForgotpasswordServlet?successfp=1");

			} else {
				response.sendRedirect(request.getContextPath() + "/HomeKhachHangServlet?errorfp=1");
			}
		} catch (Exception e) {
			e.printStackTrace();
			response.sendRedirect(request.getContextPath() + "/HomeKhachHangServlet?errorfp=1");
